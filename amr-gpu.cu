...
bool checkIfExists(const idx4& idx_cell, cuco::static_map<idx4, Cell> &hashtable) {
    // TODO modify for cuco hashtable
...
void makeBaseGrid(Cell (&grid)[N_cell_max], cuco::static_map<idx4, Cell> &hashtable) {
    ...
    for (int L = 0; L <= L_base; L++) {
        for (int hindex = 0; hindex < pow(2, N_dim * L); hindex++) {
            ...
            setGridCell(idx_cell, hindex, L == L_base, hashtable);
        }
    }
}
...
void setChildrenHelper(idx4 idx_cell, short i, cuco::static_map<idx4, Cell> &hashtable) {
    if (i == N_dim) {
        int hindex;
...
void calcGrad(cuco::static_map<idx4, Cell> &hashtable) {
    ...
    for (hashtable_itr = hashtable.begin(); hashtable_itr != hashtable.end(); hashtable_itr++) {
        ...
        calcGradCell(idx_cell, cell, hashtable);
        hashtable[idx_cell] = cell;
    }
}

void writeGrid(cuco::static_map<idx4, Cell> &hashtable) {
...
void refineGrid1lvl(cuco::static_map<idx4, Cell> &hashtable) {
    idx4 idx_cell;
    Cell cell;
...
int main() {
    ...
    idx4 empty_idx4;
    for (short i = 0; i < N_dim; i++) empty_idx4.idx3[i] = -1;
    empty_idx4.L = -1;
    
    Cell empty_cell;

    empty_cell.rho = -1;
    empty_cell.flag_leaf = 2;
    for (short i = 0; i < N_dim; i++) empty_cell.rho_grad[i] = -1;
    
    idx4 empty_idx4_sentinel = static_cast<idx4>(empty_idx4);
    Cell empty_cell_sentinel = static_cast<Cell>(empty_cell);
    cuco::static_map<idx4, Cell> hashtable{N_cell_max, cuco::empty_key{empty_idx4_sentinel}, cuco::empty_value{empty_cell_sentinel}};
